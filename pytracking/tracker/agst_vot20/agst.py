...
class AGST(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if pred_segm_region is not None:
            ...
            if hasattr(self, 'pos_iounet'):
                ...
                new_state = torch.cat((self.pos_iounet[[1,0]] - (self.target_sz_iounet[[1,0]]-1)/2, self.target_sz_iounet[[1,0]])).tolist()        
        '''
        # ------- BOUNDING BOX REFINE ------- #   
        #''' 
        new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]])).tolist()  
        ...
        if need_refine_flag:
            ...
            if pred_segm_region is not None:
                ...
                return final_mask
        #'''
        #return IoUNet refine results when segmentation results is None 
    ...
    def segment_target(self, image, pos, sz):
        ...
        if len(cnt_area) > 1 and len(contours) != 0 and np.max(cnt_area) > 50: 
            ...
            if neg_th>0.5:  # Ratio of the largest region to the second largest region 
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  
            ...
            if self.params.segm_optimize_polygon:
                ...
                prbox_opt_ = fit_bbox_to_mask(mask.astype(np.int32), rotated=False)
