...
class AGST(BaseTracker):
    ...
    def update_segm_sample(self, image, pos, sz):
        ...
        segm_pred = self.segm_net.segm_predictor(test_feat_segm, self.train_feat_segm, train_masks, test_dist_map, self.segm_feat_ups, self.up_masks, segm_update_flag=False)
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:
            ...
            if len(self.up_masks) == 3:   # size of memory bank is set to 3, if the number of samples is exceeded, pop the earliest one
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if uncert_score < self.params.tracking_uncertainty_thr:
            ...
            if (hard_negative or self.segm_hard_neg):
                if (self.frame_num-self.segm_update_num)>(self.params.segm_update_skip-1):  
                    ...
                    self.update_segm_sample(image, new_pos, self.target_sz)
                    self.segm_update_num = self.frame_num
            elif (self.frame_num<5) or ((self.frame_num-self.segm_update_num)>(2*self.params.segm_update_skip-1)): 
                # update the first 4 frames or update 20 skipping
                self.update_segm_sample(image, new_pos, self.target_sz)
                self.segm_update_num = self.frame_num   
            '''
        ...
        if pred_segm_region is not None:
            return {'target_bbox': pred_segm_region}
        # Return new state
        ...
        #''' 
        new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]])) 
        self.target_sz1 = self.pre_target_sz.clone()
        ...
        if pred_segm_region is not None:
            #self.pre_target_sz[0],self.pre_target_sz[1] = pred_segm_region[3],pred_segm_region[2]
            sz_ch_segm   = ((pred_segm_region[2]*pred_segm_region[3])/self.pre_target_sz.prod()-1).abs()
            need_refine_flag = bool(sz_ch_segm > self.params.iou_refine_thr) ## bounding box need refine if the predicted target size change greatly
        else:
            need_refine_flag = True
        ...
        if need_refine_flag:
            ...
            if hasattr(self, 'pos_iounet'):
                ...
                self.pre_target_sz[0],self.pre_target_sz[1] = self.target_sz_iounet[0],self.target_sz_iounet[1]
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        if init_mask is not None:
            ...
            if hasattr(self, 'gt_poly'):
                ...
                im1 = copy.copy(image)
                #pdb.set_trace()
                pp1=(round(p1[0]),round(p1[1]))
                pp2=(round(p2[0]),round(p2[1]))
                pp3=(round(p3[0]),round(p3[1]))
                pp4=(round(p4[0]),round(p4[1]))
                cv2.line(im1,pp1,pp2,(0,255,0),2) # draw bounding box
                cv2.line(im1,pp2,pp3,(0,255,0),2)
                cv2.line(im1,pp3,pp4,(0,255,0),2)
                cv2.line(im1,pp4,pp1,(0,255,0),2)
                patch1, f1_ = prutils.sample_target(im1, np.array(bb), self.params.segm_search_area_factor,output_sz=self.params.segm_output_sz) # sample and resize 384x384
    ...
    def segment_target(self, image, pos, sz):
        ...
        # take only the positive channel as predicted segmentation mask
        mask = F.softmax(segm_pred, dim=1)[0, 0, :, :].cpu().numpy()  
        # tmpmask = segm_pred[0, 0, :, :].cpu().numpy() # for visualization
        mask = (mask > self.params.segm_mask_thr).astype(np.uint8)
        ...
        if len(cnt_area) > 1 and len(contours) != 0 and np.max(cnt_area) > 50: 
            ...
            if neg_th>0.2:  # Ratio of the largest region to the second largest region 
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  
            ...
            if self.params.segm_optimize_polygon:
                ...
                #    mask = np.zeros(mask.shape, dtype=np.uint8)
                #    cv2.drawContours(mask, [contour], -1, 1, thickness=-1)
                #    final_mask = resize_mask(mask, segm_crop_sz, bb, image.shape[1], image.shape[0]) #######################################################
            ...
            if not self.params.segm_scale_estimation or pixels_ratio < self.params.segm_pixels_ratio:
                ...
                if True: #self.frame_num>610:
                    ...
                    heatmap = tmpmask-tmpmask.min() # show heatmap mask
                    heatmap = np.uint8(heatmap/heatmap.max()*255)#heatmap/heatmap.max()
                    #plt.figure(figsize=(10,10))
                    #plt.imshow(cv2.applyColorMap(heatmap,1))
                    plt.figure(figsize=(10,10))
                    plt.imshow(cv2.applyColorMap(heatmap,5))
                    #heatmap = cv2.applyColorMap(heatmap,cv2.COLORMAP_JET)
                    #plt.figure(figsize=(10,10))
                    #plt.imshow(heatmap)
                    plt.figure(figsize=(10,10))
                    #patch1[:,:,0] = patch1[:,:,0]*0.5 + heatmap*100
                    #patch1[:,:,1] = patch1[:,:,1]*0.5 + heatmap*100
                    #patch1[:,:,2] = patch1[:,:,2]*0.5 + heatmap*100
                    #patch1[:,:,0] = patch1[:,:,0]*(1-mask) + patch1[:,:,0]*mask*0.5 # add mask to image
                    #patch1[:,:,1] = patch1[:,:,1]*(1-mask) + patch1[:,:,1]*mask*0.5+mask*50
                    #patch1[:,:,2] = patch1[:,:,2]*(1-mask) + patch1[:,:,2]*mask*0.5
                    #patch1 = cv2.drawContours(patch1, [contour], -1, [0,255,0], thickness=1)
                    plt.imshow(patch1)
                    plt.xticks([])
                    plt.yticks([])
