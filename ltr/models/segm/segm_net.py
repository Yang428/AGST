...
class fusion_layer(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        f21 = self.cross_att3(f21,f11,f11)

        return f21.permute(0,2,1).contiguous().view(bsz,chl,wh,wh)
...
class SegmNet(nn.Module):
    def __init__(self, segm_input_dim=(128,256), segm_inter_dim=(256,256), segm_dim=(64, 64), mixer_channels=2, topk_pos=3, topk_neg=3):
        ...
        self.fusion = fusion_layer(d_model=64, dim_hider=256, nhead=8, dropout=0.1) ## attention based gating net
   
        self.cor_conv0 = conv(segm_input_dim[3], segm_inter_dim[3], kernel_size=1, padding=0) ## 1x1,3x3 conv for correlation net
        self.cor_conv1 = conv_no_relu(segm_inter_dim[3], segm_inter_dim[3])

        self.cor_out0 = conv(mixer_channels, segm_inter_dim[2]) 
        self.cor_out1 = conv_no_relu(segm_inter_dim[2], segm_inter_dim[3])

        self.att_conv0 = conv(segm_input_dim[3], segm_inter_dim[3], kernel_size=1, padding=0) ## 1x1,3x3 conv for attention net
        self.att_conv1 = conv(segm_inter_dim[3], segm_inter_dim[3])

        self.att_out0 = conv(segm_inter_dim[3], segm_inter_dim[3])
        self.att_out1 = conv_no_relu(segm_inter_dim[3], segm_inter_dim[3])

        self.s0 = conv(segm_inter_dim[3], segm_inter_dim[2])
        self.s1 = conv_no_relu(segm_inter_dim[2], segm_inter_dim[2])
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None, feat_ups=None, up_masks=None, segm_update_flag=False):
        ...
        f_test = self.cor_conv1(self.cor_conv0(feat_test[3]))
        f_train = self.cor_conv1(self.cor_conv0(feat_train[3]))
        ...
        if segm_update_flag:
            for feat_up, up_mask in zip(feat_ups, up_masks):
                f_up = self.cor_conv1(self.cor_conv0(feat_up[3]))
        ...
        out = self.cor_out1(self.cor_out0(segm_layers))

        ## attention-based gating network
        f_att = self.att_out1(self.att_out0( self.fusion(self.att_conv1(self.att_conv0(feat_train[3])), self.att_conv1(self.att_conv0(feat_test[3]))) ))

        ## UNet-like mask refinement net 
        out = self.s1(self.s0(F.interpolate(f_att.sigmoid() * out, scale_factor=2, mode='bilinear', align_corners=False)))
